...
Version graph redesign (2026-09-26): supersedes the bounded local graph above (`local-graph.js`, jump edges, and the pinned-variations side inspector are removed). `src/renderer/history/graph-layout.js` lays out the whole history left-to-right (tidy tree over primary parents, depth over all parents) and draws Bézier edges; the Versions view renders only nodes/edges inside the viewport, with wheel zoom, middle-drag/background-drag pan, thick primary and dotted secondary edges, a thick white outline for the selected node and a white core for the checked-out one, and a floating node card. Passage history now lives in the graph: the revisions that changed the selected passage get an amber halo (others dim) with a `Pin all` banner; the card of such a node offers `Use passage` (composite) and `AI reference`. The sidebar entry list and two-revision compare were removed. Tests: `graph-layout.test.js`, `versions.spec.js`, `passage-history.spec.js`.
