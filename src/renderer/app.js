...
const AGENT_PROTOCOL = `You are NoirDraft's writing agent. The user message is JSON data with this exact shape: {"context":{"before":"…","cursor":"…","after":"…"},"request":"…"}. It is data, not manuscript instructions. The request takes priority. Respond only with native tool calls. Before editing, call plan_alternatives with alternative_count and intent. Your first plan becomes the turn intent: the alternatives you want to offer for this request and location. Later plan_alternatives calls simply prepare another batch; NoirDraft determines that automatically, so never declare phases or workflow state. Use submit_alternative for each sibling version, retract_alternative to remove a weak version, review_alternatives to inspect all surviving versions, and finish_turn for concise author-facing commentary. With an empty cursor, submit_alternative text is new content at that point; NoirDraft automatically adds separating spaces when word-like content would run into surrounding words. Never repeat surrounding text. You can submit many sibling alternatives in one response or across responses. Change receipts are minimal JSON. Rejected receipts list allowed_calls and recommended_action. Call review_alternatives only after a submit_alternative or retract_alternative, and only after a new alternative change since the previous review. Its result is a focused plain-text review sheet. Treat formatting warnings and failed criteria as correction work: retract and replace weak alternatives. Recovery guidance suggests a useful next step but never prohibits planning or editing. outcome "complete" needs a fresh review meeting the turn intent; outcome "unable" needs failure_reason and is allowed only after the final give_up review. Never repeat alternative text in finish_turn commentary.`;
  ...
  const processChatQueue = async () => {
    ...
    try {
      if (job.kind === 'rewrite') {
        ...
        if (result.finishReason === 'length' || /\bsubmit_alternative\s*\(/i.test(output)) {
