  ...
  const processChatQueue = async () => {
    ...
    try {
      if (job.kind === 'rewrite') {
        ...
        if (finishReason === 'length' || /\b(?:initialize_changes|edit_notebook|review_notebook|save_notebook)\s*\(/i.test(output)) {
